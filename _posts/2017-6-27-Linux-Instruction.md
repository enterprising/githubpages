...
xargs是给命令传递参数的一个过滤器，也是组合多个命令的一个工具。它把一个数据流分割为一些足够小的块，以方便过滤器和命令进行处理。一般和 find 一起用。

wc 的意思是 word count

## 统计文本/代码的行数、字数、字节数、文件数
...
> ➜  _posts git:(master) ✗ find . -name "*.md" | xargs wc -m
>
>  8716 ./2017-7-26-Java-Timer.md 
>
>  12923 ./2017-7-26-java8-lamnda.md
>
>  15052 ./2017-8-14-Collection-Map.md
>
> 中间太长了，省略。
>
>   234214 total

可以看到，已经23万了。查看字节数的话是：404516 total。一般一个汉字=2字节（GBK编码），一个汉字=3字节（UTF-8编码）。应该有20万了吧.. 哈哈哈。继续努力。
